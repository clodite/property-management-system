...
struct strbook* view_my_bookings(char my_id[30]);    // 查询某人所有预约
struct strbook* view_all_bookings(long long book_id, int time, int roomtype, int last, char id[30], int status);   // 查询所有预约
// count_all_bookings(); // 统计所有预约
// sort_all_bookings(int order); // 排序所有预约
