...
int count_bookings(long long book_id, int time, int roomtype, int last, char id[30], int status); // [预约] 次数统计：-1不指定
int count_bookings_range(long long m_book_id, long long M_book_id, int m_time, int M_time, int m_roomtype, int M_roomtype, int m_last, int M_last, char m_id[30], char M_id[30], int status); // [预约] 范围次数统计：-1不指定
